...
Simulates the same MLFQ logic as factory.c / scheduler.c:
  ...
  • MLFQ demotion after quantum expires
  • Aging: READY process waiting ≥ 5 ticks is promoted one level
        ...
        while completed < n and not self._stop_evt.is_set():
            ...
            for p in self.procs:
                if p.state == State.READY:
                    p.rdy_wait += 1
                    if p.rdy_wait >= AGING_LIMIT and p.priority < PRIORITY_LEVELS - 1:
                        old_pr = p.priority
            ...
            for pr in range(PRIORITY_LEVELS - 1, -1, -1):
                ...
                self._log(f"  {STAGE_NAMES[pr]:<10}: {items}", G_DIM)

            # ── 4. Select highest-priority READY process ──────────────────
            pid_run = -1
            stage   = -1
            for pr in range(PRIORITY_LEVELS - 1, -1, -1):
                if self.qs[pr]:
                    pid_run = self.qs[pr].pop(0)
                    stage   = pr
                    break
            ...
            self._log(
                f"[{STAGE_NAMES[stage]:<10}] ▶ P{pid_run}  "
                f"remaining={p.remaining}  slice_left={p.slice_rem}", G_MID)
...
class FactorySchedulerApp:
    ...
    def _build_left_panel(self, parent: tk.Frame) -> None:
        ...
        tk.Label(ctrl, text="Time Quantum:", font=FNT_S,
                 bg=PANEL_BG, fg=G_DIM, anchor="w").pack(fill="x")
        self._ts_var = tk.IntVar(value=2)
    ...
    def _open_stats_window(self, data: dict) -> None:
        ...
        for label, val in [
            ("Avg Waiting Time:",    data["avg_w"]),
            ("Avg Turnaround Time:", data["avg_t"]),
            ("Avg Response Time:",   data["avg_r"]),
        ]:
            ...
            tk.Label(row, text=f"{val:.2f} ticks", font=FNT_MB,
