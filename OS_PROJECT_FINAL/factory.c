    ...
    while (1) {
        ...
        for (int i = 0; i < g_processCount; i++) {
            if (g_processes[i].state == READY) {
                ...
                if (g_processes[i].readyWaitTime >= AGING_LIMIT &&
         ...
         */
        char snapBuf[PRIORITY_LEVELS][256];

        pthread_mutex_lock(&g_mutex);
        for (int p = PRIORITY_LEVELS - 1; p >= 0; p--) {
            int n = snprintf(snapBuf[p], 256, "  Q%d [%-9s]: ", p, STAGE_NAMES[p]);
            if (isEmpty(&g_queues[p])) {
                snprintf(snapBuf[p] + n, 256 - n, "(empty)");
            } else {
                for (int j = g_queues[p].front; j <= g_queues[p].rear; j++)
                    n += snprintf(snapBuf[p] + n, 256 - n,
...
static void *workstationFn(void *arg) {
    ...
    safePrintf("[%s] Workstation thread started.\n", STAGE_NAMES[prio]);
    ...
    while (1) {
        ...
        /* ── Dequeue the part under mutex ────────────────────────────── */
        pthread_mutex_lock(&g_mutex);
        int pid = dequeue(&g_queues[prio]);
        if (pid == -1) {
        ...
        g_processes[pid].timeSliceRemaining = g_timeSlice;
        pthread_mutex_unlock(&g_mutex);
        ...
        safePrintf("[%s] ▶ Picked up P%d  (burst left: %d, quantum: %d)\n",
        ...
        while (!taskDone) {
            ...
            if (g_ganttSize < GANTT_MAX) {
                ...
                g_ganttStage[g_ganttSize] = prio;
                g_ganttTick [g_ganttSize] = tickNow;
                g_ganttSize++;
            }
            pthread_mutex_unlock(&g_mutex);
...
void factoryPrintStats(void) {
    ...
    printf("\nGantt Chart (per workstation — each cell = one tick):\n");
    printf("──────────────────────────────────────────────────────\n");
